...
/**
 * Get SHA512 hash from utf8-encoded string data
 * @param messages
 * @returns SHA512 hash in Uint8Array (length 64)
 */
export function utilSha512(message: string) {
    if (isNodeJs) {
...
function jsSha512(str) {
    ...
    function str2binb(str) {
        ...
        for (let i = 0; i < len; i += charsize) {
            bin[i >> 5] |=
                (str.charCodeAt(i / charsize) & mask) <<
                (32 - charsize - (i % 32))
        }
        return bin
    }
    ...
    str = utf8_encode(str)
    const strlen = str.length * charsize
    str = str2binb(str)
    ...
    const hexStr = binb2hex(binarray)
    return new Uint8Array(
